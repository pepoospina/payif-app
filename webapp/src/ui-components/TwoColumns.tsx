...
export const TwoColumns = (props: ITwoColumns): JSX.Element => {
  ...
  const config = ((size: string): { direction: string; widths: string[]; pad: string } => {
    const small = {
      direction: 'column',
      widths: ['100%', '100%'],
      pad: '0px',
  ...
  return (
    ...
    >
      ...
      <Box style={{ width: config.widths[0], paddingRight: config.pad }}>{children[0]}</Box>
      <Box style={{ width: config.widths[1], paddingLeft: config.pad }}>{children[1]}</Box>
    </Box>
  )
}
