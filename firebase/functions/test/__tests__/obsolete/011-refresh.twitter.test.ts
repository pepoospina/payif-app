...
describe('011-twitter refresh', () => {
  ...
  const services = getTestServices({
    time: 'mock',
    twitter: USE_REAL_TWITTER ? undefined : { signup: true },
  });
  ...
  describe('refresh token through fetchPost', () => {
    it('refresh token', async () => {
      ...
      const tweet = await twitterService.getPosts(
        [tweetId1 as string],
      ...
      const tweet2 = await twitterService.getPosts(
        [tweetId2 as string],
      ...
      const tweet3 = await twitterService.getPosts(
        [tweetId3 as string],
