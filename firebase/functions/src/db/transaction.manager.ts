...
import { applyFieldValues } from './transaction.manager.utils';
...
export class TransactionManager {
  ...
  private getCumMutationOf(id: string) {
    ...
    const obj = mutations.reduce(
      (_obj: Record<string, any>, mut) => {
        switch (mut.type) {
          ...
          case MutationType.update:
            if (mut.data) {
              Object.entries(mut.data).forEach(([key, value]) => {
                // Handle special Firestore field values
                if (
                  value &&
                  typeof value === 'object' &&
                  value !== null &&
                  ...
                  applyFieldValues(key, value, _obj);
                } else {
                  _obj[key] = value;
                }
              });
    ...
    this.mutations.forEach((mutations) => {
      ...
      for (const mut of mutations) {
        try {
          ...
          logger.error(
            `${mut.type} mutation failed for doc: ${mut.documentRef.id}`,
            {
              data: mut.data,
            }
          );
