...
export async function processInGroups<T, R>(
  inputs: T[],
  groupProcessFunc: (input: T[]) => Promise<R>,
  groupSize: number
): Promise<R[]> {
  const results: R[] = [];
  ...
  if (DEBUG)
    logger.debug(
      `Processing ${inputs.length} inputs in groups of ${groupSize}`
    );

  for (let i = 0; i < inputs.length; i += groupSize) {
    if (DEBUG)
      logger.debug(
        `Processing group ${Math.floor(i / groupSize) + 1} of ${Math.ceil(inputs.length / groupSize)}`
      );

    const group = inputs.slice(i, i + groupSize);
    const groupResult = await groupProcessFunc(group);
